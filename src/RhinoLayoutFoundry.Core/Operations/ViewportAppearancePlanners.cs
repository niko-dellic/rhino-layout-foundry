...
public sealed record SetLayoutTemplateRegistrationRequest(
    uint DocumentRuntimeSerialNumber,
    long SourceRevision,
    HierarchyScope Source,
bool Registered);

public sealed class SetLayoutTemplateRegistrationPlanner : IOperationPlanner<SetLayoutTemplateRegistrationRequest>
{
    public OperationPlan Plan(SetLayoutTemplateRegistrationRequest request, DocumentSnapshot snapshot)
    {
        ArgumentNullException.ThrowIfNull(request);
        ArgumentNullException.ThrowIfNull(snapshot);
        ...
        if (request.Source.Kind is not (HierarchyScopeKind.Sheet or HierarchyScopeKind.Detail))
            diagnostics.Add(SetHierarchyViewportRulesPlanner.Error(
                "template.source_kind", "Only layouts and details can be registered as templates."));

        var existing = snapshot.TemplateRegistrations.LastOrDefault(item => item.Source == request.Source);
        var next = !request.Registered ? null
            : new LayoutTemplateRegistration(existing?.Id ?? Guid.NewGuid(), request.Source);
        IReadOnlyList<OperationChange> changes = diagnostics.Any(item => item.Severity == DiagnosticSeverity.Error)
            ? []
            : [new SetLayoutTemplateRegistrationChange(request.Source, existing, next)];
        return new OperationPlan(snapshot.DocumentRuntimeSerialNumber, snapshot.Revision,
            "Update layout template registration", changes, diagnostics);
    }
}
