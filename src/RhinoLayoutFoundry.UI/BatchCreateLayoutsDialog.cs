...
internal sealed class BatchCreateLayoutsDialog : Dialog
{
    ...
    private const string NoLayerStateTemplate = "Inherit layer state from folder";
    private const string NoObjectDisplayTemplate = "Inherit object display state from folder";
        ...
        IReadOnlyList<BatchTarget>? editTargets)
    {
        ...
        _patternHelpButton = new FoundryToolbarIconButton(
            FoundryViewIcons.NamingPatternHelp(),
        ...
        _layerStateTemplateByLabel = AppearanceStateChoices(
            snapshot, AppearanceStateKind.LayerState);
        _objectDisplayTemplateByLabel = AppearanceStateChoices(
            snapshot, AppearanceStateKind.ObjectDisplayState);
        ...
        Items =
        {
            new Scrollable
            {
                ...
                Content = new StackLayout
                {
                    ...
                    Items =
                    {
                        FixedWidthCard("Batch", CreateBatchEditor(), 320),
                        FixedWidthCard("Page size", CreatePaperEditor(), 240),
                        FixedWidthCard("Layout", CreateLayoutEditor(), 320),
    ...
    private Control CreateBatchEditor()
    {
        ...
        table.Rows.Add(new TableRow(new StackLayout
        {
            ...
            VerticalContentAlignment = VerticalAlignment.Center,
            Spacing = FoundryTheme.Space1,
            Items = { new Label { Text = "Name / pattern" }, _patternHelpButton },
        },
    ...
    private Control CreateLayoutEditor()
    {
        ...
        else
        {
            ...
            editor.Items.Add(SectionLabel("Appearance states"));
    ...
    private Control CreatePaperEditor()
    {
        ...
        if (_isEditMode) editor.Items.Add(_paperChangeCheck);
    ...
    private void RefreshPreview(bool refreshDetailAssignments = true)
    {
        ...
        var allRows = changes.Select((change, index) => new CreationPreviewRow(
            ...
            CapabilityTemplateLabel(
                _drafts[index].LayerStateId, _layerStateTemplateByLabel),
            CapabilityTemplateLabel(
                _drafts[index].ObjectDisplayStateId, _objectDisplayTemplateByLabel))).ToArray();
    ...
    private void ApplyCapabilityTemplatesToTargets()
    {
        ...
        {
            LayerStateId = layerSource,
            ObjectDisplayStateId = objectSource,
        });
    }
    ...
    private void RefreshDetailAssignments()
    {
        ...
        for (var detailIndex = 0; detailIndex < details.Count; detailIndex++)
        {
            ...
            states[detailIndex] = new DetailPreviewState(
                ...
                namedViewLabel,
                displayModeLabel,
                mixedNamedViews ? null : _namedViewPreviewTray.PreviewFor(namedViewValues[0]),
    ...
    private void LoadEditors(CreationDraft draft)
    {
        ...
        try
        {
            ...
            _displayModePicker.Text = draft.PageDisplayModeId is { } modeId
                ...
                : InheritDisplayMode;
            _layerStateTemplatePicker.Text = draft.LayerStateId is { } layerTemplateId
                ? _layerStateTemplateByLabel.FirstOrDefault(pair => pair.Value == layerTemplateId).Key ??
                  NoLayerStateTemplate
                : NoLayerStateTemplate;
            _objectDisplayTemplatePicker.Text = draft.ObjectDisplayStateId is { } objectTemplateId
    ...
    private static Dictionary<string, Guid> AppearanceStateChoices(
        DocumentSnapshot snapshot,
        ...
        Guid? ObjectDisplayStateId)
    {
        internal LayoutCreationSpec ToSpec() => new(
            ...
            ObjectDisplayStateId: ObjectDisplayStateId);
    }
    ...
    private sealed class NamedViewPreviewTray : Drawable
    {
        ...
        internal Bitmap? PreviewFor(string? name) => string.IsNullOrWhiteSpace(name)
            ? null
            : _previews.GetValueOrDefault(name);
    ...
    private sealed class LayoutPickerDrawable : Drawable
    {
        ...
        private void OnPaint(object? sender, PaintEventArgs eventArgs)
        {
            ...
            var parts = _choices[_selectedIndex].Label.Split([" — "], 2, StringSplitOptions.None);
            graphics.DrawText(_titleFont, Enabled ? FoundryTheme.PrimaryText : FoundryTheme.MutedText,
                10, parts.Length > 1 ? 8 : 16,
                LayoutPreviewTray.FitText(graphics, _titleFont, parts[0], Math.Max(20, Width - 40)));
            if (parts.Length > 1)
                graphics.DrawText(_subtitleFont, FoundryTheme.MutedText, 10, 26,
                    LayoutPreviewTray.FitText(graphics, _subtitleFont, parts[1], Math.Max(20, Width - 40)));
            graphics.DrawText(SystemFonts.Default(10), FoundryTheme.MutedText,
                Math.Max(16, Width - 22), 15, _expanded ? "▴" : "▾");
        }
    }
