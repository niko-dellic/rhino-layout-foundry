...
| --- | --- |
| Metadata | Empty/current state save/reopen and Save As; protected rejection of schema 16 without migration; future version; malformed JSON; missing/null collections; envelope/payload mismatch. Opening/reading does not change the Rhino document. Protected metadata remains preserved on save and Foundry rejects changes. |
...
Use fresh fixtures created by this candidate. Schema 17/package 6 are required, plus one schema-16 protected-rejection fixture; other historical-project compatibility is outside acceptance.
