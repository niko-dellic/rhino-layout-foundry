...
Folder and sheet records own nullable `CreatedUtc` and `LastModifiedUtc` values. New records receive one UTC timestamp for both fields. Successful Foundry edits preserve creation and advance modification at the mutation boundary, including page-name, paper, detail, appearance, title-block, print, notes, and hierarchy changes stored natively by Rhino. For schema 16 documents, the migration uses the 3DM file creation and modification dates as the best recoverable baseline because exact historical entity dates were never stored. Dates remain unchanged on failed operations and participate in metadata Undo/Redo with the rest of the immutable document state.
