...
Folder and sheet records own nullable `CreatedUtc` and `LastModifiedUtc` values. New records receive one UTC timestamp for both fields. Successful Foundry edits preserve creation and advance modification at the mutation boundary, including page-name, paper, detail, appearance, title-block, print, notes, and hierarchy changes stored natively by Rhino. Schema 16 is unsupported and preserved without migration; only schema 17 is editable. Dates remain unchanged on failed operations and participate in metadata Undo/Redo with the rest of the immutable document state.
