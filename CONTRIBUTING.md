...
Fully quit Rhino, run `./scripts/install-dev-macos.sh Debug`, compare SHA-256 hashes of source and installed binaries, and reopen Rhino. An optional second argument to `install-dev-macos.sh` selects a verified isolated output directory directly. The installer refuses to run while Rhino is open. Installation is not live assembly reload. On Windows use the development load directory documented in README.

For package install/uninstall/reinstall testing, use [Developer installation testing](docs/DEVELOPER_INSTALL_TESTING.md). The repository-only macOS helper parks/restores both development bundles with Rhino closed. A separate OS user account is preferred for first-install qualification.
...
Explain native workarounds, ownership, cancellation, and failure behavior near the code. Update the canonical document when changing a contract, and add changelog entries for observable behavior. Maintain document schema 17 and package format 6. Schema 16 is protected and unsupported; do not migrate or reset it. Do not add historical converters or compatibility aliases. Preserve generic invalid/unsupported-state guards and original-envelope pass-through.
